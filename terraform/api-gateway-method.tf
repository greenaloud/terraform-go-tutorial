resource "aws_api_gateway_resource" "note" {
  ...
  parent_id   = aws_api_gateway_rest_api.wocho-test.root_resource_id
  path_part   = "note"
}
...
resource "aws_api_gateway_integration" "note" {
  ...
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:ap-northeast-2:lambda:path/2015-03-31/functions/${aws_lambda_function.wocho-test.arn}/invocations"
}
