...
func getNote(user string, id string) (*note, error) {
	result, err := noteTable.GetItem("user", user, NoteKey{User: user, Id: id})
	if err != nil {
		return nil, err
	}

	note := note{}
	dynamodbattribute.UnmarshalMap(result.Items[0], &note)
